...
if( ~isfield(pChns,'complete') || pChns.complete~=1 || isempty(I) )
  p=struct('enabled',{},...
      ...
      'pFunc',{},...
      'padWith',{});
  pChns = getPrmDflt(varargin,{'shrink',4,...
      'pDisparity',{},...
      'pColor',{},...
      'pGradMag',{},...
      'pGradMagDisp',{},...
  ...
  pChns.pDisparity = getPrmDflt( pChns.pDisparity,...
      {'enabled', 1,...
       'maxDepth', 40,...
       'periodicalFlagU16', 8192,...
       'uniquenessFlagU16', 4096,...
       'valueMaskU16', 2047,...
       'floatFactor', 0.0625,...
       'baseline', 0.1200007,...
       'focalLength', 1378.359985}, 1);
  
  pChns.pColor = getPrmDflt( pChns.pColor,...
      {'enabled',1,...
       'smooth',1,... 
       'colorSpace','luv'}, 1 );
  ...
  pChns.pGradMagDisp = getPrmDflt( pChns.pGradMagDisp,...
      {'enabled',1,...
       ...
       'normConst',.005,...
       'full',0}, 1 );
  pChns.pGradHist = getPrmDflt( pChns.pGradHist,...
      {'enabled',1,...
       'binSize',[],...
       'nOrients',6,...
       'softBin',0,...
...
if size(I, 3) == 4
    ImgGray = I(:,:,1:3);
    ImgGray = rgbConvert(ImgGray, p.colorSpace);
    ImgDisp = I(:,:,4);
elseif size(I, 3) == 2
    ImgGray = I(:,:,1);
    ImgDisp = I(:,:,2); 
...
p = pChns.pGradMag; 
nm = 'gradient magnitude gray image';
full = 0; 
if(isfield(p,'full'))
    full = p.full; 
end
...
p = pChns.pGradHist; 
nm = 'gradient histogram gray image';
if( p.enabled )
  binSize = p.binSize; 
  if(isempty(binSize))
      binSize = shrink; 
...
for i=find( [p.enabled] )
  C = feval(p(i).hFunc, ImgGray, p(i).pFunc{:});
